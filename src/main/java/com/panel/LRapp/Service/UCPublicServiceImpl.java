...
public class UCPublicServiceImpl implements UCPublicService{
    ...
    public UCPublicResponse save(UserChallengePublic userChallengePublic,String token) {
        ...
        return new UCPublicResponse("تمت الاضافه بنجاح :)",ucPublicRepo.save(userChallengePublic),user.getName(),user.getImage());
    }
    ...
    public UCPublicResponse update(UserChallengePublic userChallengePublic, String token) {
        ...
        if(userChallengePublic1.isEmpty()){
            return new UCPublicResponse("لم يتم العثور على هذا التحدي :(",null,null,null);
        }else{
            ...
            return new UCPublicResponse("تم التعديل بنجاح :)",ucPublicRepo.save(userChallengePublic1.get()),user.getName(),user.getImage());
        }
    }
